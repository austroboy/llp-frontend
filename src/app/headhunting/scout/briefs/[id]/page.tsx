...
export default function BriefDetailPage() {
  ...
  const slotInfo = useQuery(
    api.headhunting.scoutQueue.getSlotInfo,
    userId && data?.release?.mandateId
      ? { scoutId: userId, mandateId: data.release.mandateId }
      : "skip"
  );
  ...
  const handleSubmitAndAssess = async () => {
    ...
    try {
      const submissionId = await submitCandidate({
        mandateId: data!.release.mandateId,
        scoutId: userId,
      ...
      fireNotification("submission_received", {
        ...
        candidateName: form.candidateName,
        mandateTitle: data?.mandate?.rawTitle || "Mandate",
        mandateId: data!.release.mandateId,
      });
      ...
      if (!assessRes.ok) {
        // Assessment failed — still let scout proceed but skip review stage
  ...
  const handleProceedToConfirm = () => {
    // Validate: any override must have justification
  ...
  const handleFinalSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      // Save scout overrides to the assessment
  ...
  if (!data) {
    return <div className="mx-auto max-w-4xl py-12 text-center text-sm text-muted-foreground">Brief not found.</div>;
  }
  ...
  const slotsFull = slotInfo ? slotInfo.remaining <= 0 : false;
    ...
    <div className="mx-auto max-w-4xl space-y-6 px-4 py-8">
      ...
      <div className="rounded-lg border border-border bg-card p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-xl font-bold mb-2">{blueprint?.title ?? "—"}</h1>
            <div className="flex items-center gap-3 flex-wrap text-sm">
              {clientHint ? (
                <span className="flex items-center gap-1 text-muted-foreground">
                  <Building2 className="size-3.5" />
                  {release.disclosureLevel === "disclosed" ? clientHint : `${t("scout.company.hint")}: ${clientHint}`}
                </span>
              ) : (
                <span className="flex items-center gap-1 text-muted-foreground">
          ...
          {!showSubmitForm && (
            ...
            >
              ...
              {slotsFull ? "Slots Full" : t("scout.briefs.submitCandidate")}
            </Button>
          )}
        </div>
        ...
        {blueprint && (
          <div className="mt-4 space-y-3">
            {(blueprint.mustHaves ?? []).length > 0 && (
              <div>
                <p className="text-xs font-semibold text-muted-foreground mb-1.5">MUST HAVE</p>
                <ul className="space-y-1">
                  {(blueprint.mustHaves ?? []).map((m: string, i: number) => (
            ...
            {(blueprint.dealBreakers ?? []).length > 0 && (
              <div>
                <p className="text-xs font-semibold text-muted-foreground mb-1.5">DEAL BREAKERS</p>
                <ul className="space-y-1">
                  {(blueprint.dealBreakers ?? []).map((d: string, i: number) => (
                    <li key={i} className="flex items-start gap-1.5 text-sm">
        ...
        <div className={cn(
          ...
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium">
              Submissions: {slotInfo.used} / {slotInfo.allocated} slots used
            </span>
            {slotsFull && (
          ...
          <div className="w-full h-2 bg-muted rounded-full overflow-hidden">
            <div
              className={cn(
                ...
                slotsFull ? "bg-red-500" : slotInfo.used / slotInfo.allocated > 0.7 ? "bg-amber-500" : "bg-green-500"
              )}
              style={{ width: `${Math.min(100, (slotInfo.used / slotInfo.allocated) * 100)}%` }}
            />
          </div>
          {slotsFull && (
            <p className="text-xs text-red-600 mt-2">
              All submission slots have been used. No further candidates can be submitted for this mandate.
            </p>
          )}
        </div>
      )}
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm mt-0.5">{
                          // Find requirement label from blueprint
                          blueprint?.criticalMatchPoints?.find((_, i) => `REQ-${String(i+1).padStart(3,"0")}` === ev.requirementId) ??
                          blueprint?.mustHaves?.find((_, i) => `REQ-${String(i+1).padStart(3,"0")}` === ev.requirementId) ??
                          ev.requirementId
                        }</p>
